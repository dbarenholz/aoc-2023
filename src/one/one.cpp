...
ll floorMod(const ll a, const ll b) { return (a % b + b) % b; }
...
vec<str> input() {
  ...
  ifstream infile("/home/dan/dev/aoc-2023/src/one/input.txt");
  str line;
  if (infile.is_open()) {
    while (infile) {
      getline(infile, line);
  ...
  // remove last item that somehow gets put there twice?
  lines.pop_back();
